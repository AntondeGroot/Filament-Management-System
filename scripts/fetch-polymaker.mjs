 ...
 * colors in the file solves all three.
 *
 * The cost is that the copy goes stale when Polymaker adds a color, which is
...
/* The page renders each color as a name in a <p> followed by its hex in a
...
for (const token of tokens) {
  if (/^#[0-9A-Fa-f]{6}$/.test(token)) {
    if (pending && !colors.has(pending)) colors.set(pending, token.toUpperCase());
    pending = null;
...
if (colors.size < 100) {
  throw new Error(`only ${colors.size} colors parsed — the page layout has probably changed`);
}

const entries = [...colors].map(([name, hex]) => `"${name}":"${hex}"`);
const lines = [];
let line = "  ";
...
console.log(after === before
  ? `${colors.size} colors, unchanged since the last run`
  : `wrote ${colors.size} colors into src/polymaker.js`);