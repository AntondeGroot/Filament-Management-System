...
#   4. Installing over an app signed with a different debug key fails with
#      INSTALL_FAILED_UPDATE_INCOMPATIBLE; the fix is to uninstall first, which
#      wipes your spools, so that is never done without asking.
#
# Note on what the APK actually renders: unlike PhotoKeeper there is no
# `server.url` — the app is self-contained, so the bundle inside the APK is what
# runs and the phone needs nothing to be reachable. The flip side is that every
...
PACKAGE="uk.antondegroot.filament"
...
if [[ "${1:-}" == "--skip-build" ]]; then
  ...
  echo "→ collecting the app into dist/ and syncing it into android/"
  npm run cap:sync
...
# ——— Install ———
#
# -r reinstalls in place and keeps app data. That data is the whole inventory —
# every spool, unit and project lives in the webview's IndexedDB and nowhere
# else — so losing it is not a minor inconvenience.
...
if ! output="$("$ADB" -s "$SERIAL" install -r "$APK" 2>&1)"; then
  ...
  if [[ "$output" == *INSTALL_FAILED_UPDATE_INCOMPATIBLE* || "$output" == *INSTALL_FAILED_VERSION_DOWNGRADE* ]]; then
    fail "a different build of $PACKAGE is already installed. Removing it wipes your spools:
  $ADB -s $SERIAL uninstall $PACKAGE && npm run android:install -- --skip-build"
  fi
  fail "install failed"
fi
printf '%s\n' "$output"

printf '\nInstalled. Open Filament Manager on the phone, then Setup → Reminders → notify me.\n'