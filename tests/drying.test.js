...
/* Only the field the decision reads: whether a dry cycle is still unanswered.
   Anything else on a spool would be noise, and inventing it would suggest the
   rule depends on it. */
const roll = (id, dryPending) => ({ id, dryPending });
...
describe("needsDryingAnswer()", () => {
  it("asks only about the rolls whose drying is not already on record", () => {
    const asked = needsDryingAnswer({
      ...
      toKind: "box",
      spools: [
        roll("sp-running", true),    /* went in, nobody has said whether it finished */
        roll("sp-answered", false),  /* this visit has been answered — asking again is noise */
        roll("sp-second", true),     /* dried this morning, put back in, out again */
      ],
    });
    ...
    expect(asked).toEqual(["sp-running", "sp-second"]);
  });

  it("stays quiet when the question is not worth putting", () => {
    const rolls = [roll("sp-a", true), roll("sp-b", true)];
    const move = change =>
      needsDryingAnswer({ fromKind: "dryer", toKind: "box", spools: rolls, ...change });
